...
class POS extends Controller
{
    ...
    protected function saveOrder(): void
    {
        ...
        // Trigger onChange('total') to generate accounting entry and receipts
        if ($document->modelClassName() === 'FacturaCliente' && $document->total > 0) {
            /** @var FacturaCliente $document */
            // Use reflection to call the protected onChange('total') method
            // This will trigger onChangeTotal() which generates asientos and recibos
            $reflection = new ReflectionMethod($document, 'onChange');
            $reflection->setAccessible(true);
            if (false === $reflection->invoke($document, 'total')) {
                Tools::log('POS')->warning('fail-trigger-onchange-total');
                $this->dataBase->rollback();
                return;
            }
            // Save the document to persist any changes made by onChangeTotal
            if (false === $document->save()) {
                Tools::log('POS')->warning('fail-save-after-onchange');
                $this->dataBase->rollback();
                return;
            }
        }
